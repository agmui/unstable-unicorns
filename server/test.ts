...
if (game.players[0].getHand().length==0 || game.players[1].getHand().length == 0) console.log('ERROR: set up not drawing for a player')
console.log('---------')
//==check addCard==
setup()
...
if(deck== game.deck.length) console.log('ERROR: addCard did not work for deck')
game.addCard(cards, 'discard')
if(game.discard.length==0)console.log('ERROR: addCard did not work for discard')
console.log('---------')
//==check removeCard==
setup()
...
if(deck != game.deck.length) console.log('ERROR: removeCard did not work for deck')
game.addCard(cards, 'discard')
game.removeCard(cards, 'discard')
if(game.discard.length)console.log('ERROR: removeCard did not work for discard')
console.log('---------')
//==check move==
setup()
...
if (game.discard.length) console.log('ERROR: interupt let host go before interupt')
game.move('a', game.players[1].getHand(), ['a','Hand'], 'discard')
console.log('discard:',game.discard)
if (game.discard.length==0) console.log('ERROR: interupt did not let opponate go')
...
//wrong phase
game.rotateTurn()
if (game.turn>1) console.log('ERROR: roatateTurn changed turn on phase <5')
//rotat turn
for (let i=0;i<4;i++){
    game.rotatePhase()
}
...
if (game.turn<1) console.log('ERROR: roatateTurn does not work')
console.log('---------')
//==check rotatePhase==
setup()
console.log('check rotatePhase\n')
game.rotatePhase()
if (game.phase<1) console.log('ERROR: rotatePhase not working')
