...
if (deckValue == game.deck)
    console.log('ERROR: drawFromDeck didn\'t change deck'.bgRed);
...
if (game.discard.length != 0)
    console.log('ERROR: did not draw from discard'.bgRed);
...
if (game.players[0].getHand().length == 0 || game.players[1].getHand().length == 0)
    console.log('ERROR: set up not drawing for a player'.bgRed);
console.log('---------');
//==check addCard==
setup();
...
if (deckValue === game.deckValue)
    console.log('ERROR: addCard did not work for deck'.bgRed);
game.addCard(cards, 'discard');
if (game.discard.length == 0)
    console.log('ERROR: addCard did not work for discard'.bgRed);
console.log('---------');
//==check removeCard==
setup();
...
if (deckValue != game.deckValue.length)
    console.log('ERROR: removeCard did not work for deck'.bgRed);
game.addCard(cards, 'discard');
game.removeCard(cards, 'discard');
if (game.discard.length)
    console.log('ERROR: removeCard did not work for discard'.bgRed);
console.log('---------');
//==check move==
setup();
...
if (game.discard.length)
    console.log('ERROR: interupt let host go before interupt'.bgRed);
game.move('a', game.players[1].getHand(), ['a', 'Hand'], 'discard');
console.log('discard:', game.discard);
if (game.discard.length == 0)
    console.log('ERROR: interupt did not let opponate go'.bgRed);
//chain interupts
console.log('---------');
//==check rotateTurn==
setup();
...
game.rotateTurn();
if (game.turn > 1)
    console.log('ERROR: roatateTurn changed turn on phase <5'.bgRed);
//rotat turn
for (let i = 0; i < 4; i++) {
    game.rotatePhase();
}
...
if (game.turn < 1)
    console.log('ERROR: roatateTurn does not work'.bgRed);
console.log('---------');
//==check rotatePhase==
setup();
...
game.rotatePhase();
if (game.phase < 1)
    console.log('ERROR: rotatePhase not working'.bgRed);
